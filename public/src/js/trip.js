...
function initMap()
{
  //keeps track of whether any window is active at the moment
  ...
  var latLong = {lat:35.438465098846116, lng: 137.68374122072777};

  //Creates the map
  map = new google.maps.Map(document.getElementById("map"),{center: latLong, zoom: 8.5 });

  //This loop will go through every marker
  for(var n=0;n<markerInfo.length;n++)
    {
        ...
        const marker = new google.maps.Marker
         ({
            position: markerInfo[n].loc[0],
            map:map,
            icon: "https://img.icons8.com/plasticine/100/000000/map-pin.png"
         });
        ...
        (
            'click', function()
            {
                //previous window needs to be closed before if its open
                 if(activeInfoWindow)
                  {
...
//    export {currContent, currentPlaceName};

 //   updateBtnClicked= True;
//    currContent = document.getElementById("textArea").value;
//    contentString =  '<p>'+markerInfo[n].placeInfo+'</p>'+'<div>' + '<input id="userInput" type="text" value="'+
//    currContent + '" />' + '<input id="updateButton" type="button" value="Update" onclick="update()"/>' + '</div>';
